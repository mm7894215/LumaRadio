...
const APP_NAME = 'LumaRadio';
const APP_USER_MODEL_ID = 'com.rocky.lumaradio';
...
const NETEASE_LOGIN_PARTITION = 'persist:lumaradio-netease-login';
const NETEASE_LOGIN_URL = 'https://music.163.com/#/login';
const QQ_LOGIN_PARTITION = 'persist:lumaradio-qqmusic-login';
...
function sendGlobalHotkeyAction(action) {
  ...
  mainWindow.webContents.send('lumaradio-global-hotkey', { action });
}

function unregisterLumaRadioGlobalHotkeys() {
...
function configureLumaRadioGlobalHotkeys(bindings = []) {
  unregisterLumaRadioGlobalHotkeys();
  const results = [];
  const seen = new Set();
...
function runRendererCommand(source) {
  if (!focusMainWindow() || !source) return;
  mainWindow.webContents.executeJavaScript(String(source), true).catch((error) => {
    console.warn('Renderer command failed:', error.message);
  });
}
...
function installApplicationMenu() {
  ...
  const template = [
      ...
      submenu: [
        {
          ...
          click: () => dialog.showMessageBox({
            ...
            detail: 'Rocky 维护的独立 Web 与 macOS 沉浸式音乐播放器。\n本软件以 GNU GPL v3.0 开源。',
            buttons: ['好'],
          }),
        },
        { type: 'separator' },
        { label: '设置…', accelerator: 'Command+,', click: () => runRendererCommand('toggleFxPanel(true)') },
        { type: 'separator' },
        { role: 'services' },
        { type: 'separator' },
        { role: 'hide' },
    ...
    },
    {
      label: '播放',
      submenu: [
        { label: '播放 / 暂停', accelerator: 'CommandOrControl+Shift+P', click: () => runRendererCommand('togglePlay()') },
        { label: '上一首', accelerator: 'CommandOrControl+Shift+Left', click: () => runRendererCommand('prevTrack()') },
        { label: '下一首', accelerator: 'CommandOrControl+Shift+Right', click: () => runRendererCommand('nextTrack()') },
      ],
    },
    {
      label: '显示',
      submenu: [
        { label: '回到主页', accelerator: 'CommandOrControl+Shift+H', click: () => runRendererCommand('goHome()') },
        { label: '搜索', accelerator: 'CommandOrControl+F', click: () => runRendererCommand("document.getElementById('search-input')?.focus()") },
        { label: '视觉控制台', click: () => runRendererCommand('toggleFxPanel(true)') },
        { type: 'separator' },
    ...
    { role: 'windowMenu', label: '窗口' },
    {
      ...
      label: '帮助',
      submenu: [
        { label: '使用引导', click: () => runRendererCommand('startVisualGuide({ manual: true })') },
        {
          label: '开源许可证',
          click: () => dialog.showMessageBox({
            type: 'info',
            title: `${APP_NAME} 开源许可证`,
            message: 'GNU General Public License v3.0',
            detail: '完整许可证与必要版权声明随源代码和应用包提供。',
            buttons: ['好'],
          }),
        },
      ],
...
function shouldEnsureDesktopShortcut() {
  ...
  if (process.env.LUMARADIO_NO_DESKTOP_SHORTCUT === '1') return false;
  return app.isPackaged || process.env.LUMARADIO_CREATE_DESKTOP_SHORTCUT === '1';
}
...
function ensureDesktopShortcut() {
  ...
  try {
    ...
    const shortcut = {
      ...
      args: '',
      description: 'LumaRadio immersive music player',
...
async function openNeteaseMusicLoginWindow(owner) {
  ...
  if (neteaseLoginWindow && !neteaseLoginWindow.isDestroyed()) {
    neteaseLoginWindow.show();
    neteaseLoginWindow.focus();
    return { ok: false, pending: true, message: '网易云登录窗口已打开' };
  }
  ...
  return new Promise((resolve) => {
    ...
    neteaseLoginWindow = loginWindow;
    ...
    loginWindow.on('closed', async () => {
      if (neteaseLoginWindow === loginWindow) neteaseLoginWindow = null;
      if (settled) return;
      if (pollTimer) clearInterval(pollTimer);
      try {
        const cookie = await readNeteaseLoginCookieHeader(cookieSession);
...
async function openQQMusicLoginWindow(owner) {
  ...
  if (qqLoginWindow && !qqLoginWindow.isDestroyed()) {
    qqLoginWindow.show();
    qqLoginWindow.focus();
    return { ok: false, pending: true, message: 'QQ 音乐登录窗口已打开' };
  }
  ...
  return new Promise((resolve) => {
    ...
    loginWindow.on('ready-to-show', () => loginWindow.show());
    loginWindow.on('closed', async () => {
      if (qqLoginWindow === loginWindow) qqLoginWindow = null;
      if (settled) return;
      if (pollTimer) clearInterval(pollTimer);
      try {
        const cookie = await readQQLoginCookieHeader(cookieSession);
...
public class LumaRadioMousePoll {
  [DllImport("user32.dll")] public static extern short GetAsyncKeyState(int vKey);
}
"@
$prev = $false
while ($true) {
  $down = (([LumaRadioMousePoll]::GetAsyncKeyState(4) -band 0x8000) -ne 0)
...
function broadcastDesktopLyricsLockState() {
  ...
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('lumaradio-desktop-lyrics-lock-state', { locked });
  }
...
function broadcastDesktopLyricsEnabledState(enabled) {
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send('lumaradio-desktop-lyrics-enabled-state', { enabled: !!enabled });
  }
}
...
function sendDesktopLyricsState() {
  if (!desktopLyricsWindow || desktopLyricsWindow.isDestroyed()) return;
  desktopLyricsWindow.webContents.send('lumaradio-desktop-lyrics-state', desktopLyricsState);
}
...
function createDesktopLyricsWindow(payload = {}) {
  ...
  desktopLyricsWindow = new BrowserWindow({
    ...
    skipTaskbar: true,
    show: false,
    title: 'LumaRadio Desktop Lyrics',
    webPreferences: {
...
if (-not ("LumaRadioNativeWin" -as [type])) {
Add-Type @"
using System;
using System.Runtime.InteropServices;
public class LumaRadioNativeWin {
...
$progman = [LumaRadioNativeWin]::FindWindow("Progman", $null)
$result = [IntPtr]::Zero
[LumaRadioNativeWin]::SendMessageTimeout($progman, 0x052C, [IntPtr]::Zero, [IntPtr]::Zero, 0, 1000, [ref]$result) | Out-Null
$script:workerw = [IntPtr]::Zero
$enum = [LumaRadioNativeWin+EnumWindowsProc]{
  param([IntPtr]$top, [IntPtr]$param)
  $shell = [LumaRadioNativeWin]::FindWindowEx($top, [IntPtr]::Zero, "SHELLDLL_DefView", $null)
  if ($shell -ne [IntPtr]::Zero) {
    $script:workerw = [LumaRadioNativeWin]::FindWindowEx([IntPtr]::Zero, $top, "WorkerW", $null)
  }
  return $true
}
[LumaRadioNativeWin]::EnumWindows($enum, [IntPtr]::Zero) | Out-Null
if ($script:workerw -eq [IntPtr]::Zero) { $script:workerw = $progman }
$target = [IntPtr]::new([Int64]${hwnd})
[LumaRadioNativeWin]::SetParent($target, $script:workerw) | Out-Null
[LumaRadioNativeWin]::SetWindowPos($target, [IntPtr]::Zero, 0, 0, 0, 0, 0x0013) | Out-Null
`;
...
function sendWallpaperState() {
  if (!wallpaperWindow || wallpaperWindow.isDestroyed()) return;
  wallpaperWindow.webContents.send('lumaradio-wallpaper-state', wallpaperState);
}
...
function closeMusicLoginWindows() {
  [neteaseLoginWindow, qqLoginWindow].forEach((win) => {
...
ipcMain.handle('lumaradio-hotkeys-configure-global', (_event, bindings) => {
  return configureLumaRadioGlobalHotkeys(bindings);
});

ipcMain.handle('lumaradio-export-json-file', async (event, payload = {}) => {
  try {
    const owner = getSenderWindow(event);
    const defaultName = String(payload.defaultName || 'lumaradio-export.json').replace(/[\\/:*?"<>|]+/g, '-');
    const result = await dialog.showSaveDialog(owner, {
      title: '导出 LumaRadio 存档',
...
ipcMain.handle('lumaradio-import-json-file', async (event) => {
  try {
    const owner = getSenderWindow(event);
    const result = await dialog.showOpenDialog(owner, {
      title: '导入 LumaRadio 存档',
      properties: ['openFile'],
      filters: [{ name: 'JSON', extensions: ['json'] }],
    });
...
ipcMain.handle('lumaradio-open-update-installer', async (_event, filePath) => {
  try {
...
ipcMain.handle('lumaradio-restart-app', async () => {
  try {
    app.relaunch();
    app.exit(0);
    return { ok: true };
...
ipcMain.handle('lumaradio-desktop-lyrics-set-enabled', async (_event, enabled, payload) => {
  try {
    if (enabled) {
...
ipcMain.handle('lumaradio-desktop-lyrics-update', async (_event, payload) => {
  try {
...
ipcMain.handle('lumaradio-desktop-lyrics-set-dragging', async () => {
  return { ok: true };
});

ipcMain.handle('lumaradio-desktop-lyrics-set-pointer-capture', async (_event, active) => {
  try {
...
ipcMain.handle('lumaradio-desktop-lyrics-set-hot-bounds', async (_event, bounds) => {
  try {
...
ipcMain.handle('lumaradio-desktop-lyrics-set-lock-state', async (_event, locked) => {
  try {
...
ipcMain.handle('lumaradio-desktop-lyrics-move-by', async (_event, dx, dy) => {
  try {
...
ipcMain.handle('lumaradio-wallpaper-set-enabled', async (_event, enabled, payload) => {
  try {
...
ipcMain.handle('lumaradio-wallpaper-update', async (_event, payload) => {
  try {
...
async function createWindow() {
  ...
  windowFullscreenActive = false;
  const port = localServer && mainServerPort ? mainServerPort : await findOpenPort(3000);
  mainServerPort = port;
  ...
  process.env.LUMARADIO_UPDATE_DIR = getUpdateDownloadDir();
  try {
  ...
  mainWindow = new BrowserWindow({
    ...
    fullscreen: false,
    transparent: !isMac,
    backgroundColor: isMac ? '#050608' : '#00000000',
    hasShadow: true,
    autoHideMenuBar: true,
    title: APP_NAME,
    icon: APP_ICON,
