...
位置：`src/styles/player.css` 顶部 `:root`。

```css
:root{
  --saved-panel-glass-bg:rgba(0,0,0,.10);
  --saved-panel-glass-filter:blur(12px) saturate(1.8) brightness(1.16);
  --saved-panel-glass-svg-filter:url(#lumaradio-control-glass-filter) saturate(1);
  ...
  --saved-button-glass-svg-filter:url(#lumaradio-control-glass-filter) saturate(1);
...
位置：`src/styles/player.css` 的 `#bottom-bar`。
...
html.control-glass-svg-ok #bottom-bar{
  background:rgba(0,0,0,.10);
  backdrop-filter:url(#lumaradio-control-glass-filter) saturate(1);
  -webkit-backdrop-filter:url(#lumaradio-control-glass-filter) saturate(1);
}
...
位置：`index.html` 的 `<svg id="control-glass-svg">`。

关键点：

- `color-interpolation-filters="sRGB"`
- 主 filter：`id="lumaradio-control-glass-filter"`
...
位置：`src/runtime/90-queue.ts` 的 `generateControlGlassDisplacementMap(width, height, radius)`。
