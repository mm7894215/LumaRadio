...
var SEARCH_HISTORY_STORE_KEY = 'lumaradio-search-history';
...
function searchResultKey(q, mode) {
  if (window.LumaRadioModules && window.LumaRadioModules.search) {
    return window.LumaRadioModules.search.searchResultKey(q, window.LumaRadioModules.search.normalizeSearchMode(mode || searchMode));
  }
...
function rememberSearchQuery(q) {
  ...
  if (!q) return;
  if (window.LumaRadioModules && window.LumaRadioModules.search) {
    writeSearchHistory(window.LumaRadioModules.search.rememberQuery(readSearchHistory(), q));
    return;
  }
...
function handleSearchResultKey(event, action, index) {
  if (!event || (event.key !== 'Enter' && event.key !== ' ')) return;
  event.preventDefault();
  if (action === 'podcast') openPodcastPrograms(index);
  else if (action === 'episode') playPodcastProgram(index);
  else playSearchResult(index);
}
function renderPodcastRadios(items, label) {
  ...
  $results.innerHTML = podcastResults.map(function(p, i){
    return '<div class="search-result">' +
      '<div class="search-result-main" role="button" tabindex="0" aria-label="打开播客 ' + escHtml(p.name || '') + '" onclick="openPodcastPrograms(' + i + ')" onkeydown="handleSearchResultKey(event,\'podcast\',' + i + ')">' +
...
function renderPodcastPrograms() {
  ...
  $results.innerHTML =
    ...
    podcastPrograms.map(function(p, i){
      return '<div class="search-result">' +
        '<div class="search-result-main" role="button" tabindex="0" aria-label="播放 ' + escHtml(p.name || '') + '" onclick="playPodcastProgram(' + i + ')" onkeydown="handleSearchResultKey(event,\'episode\',' + i + ')">' +
          searchThumbHtml(p.cover) +
          '<div class="search-result-info">' +
...
function renderSongSearchResults(songs) {
  ...
  $results.innerHTML = playlist.map(function(s, i){
    ...
    return '<div class="search-result ' + sourceClass + '">' +
      '<div class="search-result-main" role="button" tabindex="0" aria-label="播放 ' + escHtml(s.name || '') + ' - ' + escHtml(s.artist || '未知歌手') + '" onclick="playSearchResult(' + i + ')" onkeydown="handleSearchResultKey(event,\'song\',' + i + ')">' +
        imgTag +
        '<div class="search-result-info">' +
