...
function initLumaRadioSplashWebgl(canvas) {
  var gl = null;
  try {
...
function drawLumaRadioSplashWebgl(elapsed) {
  var gl = splashGl;
...
(function initLumaRadioSplashCanvas() {
  splashCanvas = document.getElementById('splash-canvas');
  if (!splashCanvas) return;
  if (!reduceSplashMotion && initLumaRadioSplashWebgl(splashCanvas)) {
    splashCtx = null;
  } else {
...
function playLumaRadioIntroSound() {
  ...
  try {
    ...
    if (ctx.state === 'suspended' && ctx.resume) {
      ctx.resume().then(function(){
        if (!splashSoundPlayed) playLumaRadioIntroSound();
      }).catch(function(){});
...
function markSplashReadyToEnter() {
  ...
  s.setAttribute('aria-label', '点击进入 LumaRadio');
}
...
function currentDesktopSongMeta() {
  ...
  return {
    title: song.name || song.title || 'LumaRadio',
...
function currentDesktopLyricSnapshot() {
  ...
  return { text: normalizeDesktopLyricText(currentDesktopSongMeta().title || 'LumaRadio'), progress: 0, progressSpan: 4.8 };
}
