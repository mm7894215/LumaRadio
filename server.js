...
const UA = `Mozilla/5.0 (${process.platform === 'darwin' ? 'Macintosh; Intel Mac OS X 10_15_7' : 'Windows NT 10.0; Win64; x64'}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36`;
const WEB_DATA_DIR = process.env.LUMARADIO_DATA_DIR || path.join(os.homedir(), '.lumaradio');
const COOKIE_FILE = process.env.COOKIE_FILE || path.join(WEB_DATA_DIR, '.cookie');
const QQ_COOKIE_FILE = process.env.QQ_COOKIE_FILE || path.join(WEB_DATA_DIR, '.qq-cookie');
const UPDATE_WORK_DIR = process.env.LUMARADIO_UPDATE_DIR || path.join(WEB_DATA_DIR, 'updates');
const UPDATE_DOWNLOAD_DIR = process.env.LUMARADIO_UPDATE_DOWNLOAD_DIR || path.join(UPDATE_WORK_DIR, 'downloads');
const UPDATE_PATCH_BACKUP_DIR = process.env.LUMARADIO_PATCH_BACKUP_DIR || path.join(UPDATE_WORK_DIR, 'backups', 'patches');
const BEATMAP_CACHE_DIR = process.env.LUMARADIO_BEAT_CACHE_DIR || path.join(WEB_DATA_DIR, 'cache', 'beatmaps');
const APP_PACKAGE = readPackageInfo();
const APP_VERSION = process.env.LUMARADIO_VERSION || APP_PACKAGE.version || '1.0.0';
const UPDATE_CONFIG = readUpdateConfig(APP_PACKAGE);
const STATIC_ROOT = path.resolve(process.env.LUMARADIO_STATIC_DIR || path.join(__dirname, 'dist-web'));
...
function saveCookie(c) {
  ...
  try {
    ...
    fs.writeFileSync(COOKIE_FILE, userCookie, { mode: 0o600 });
  } catch (e) {}
}
...
function saveQQCookie(c) {
  ...
  try {
    fs.mkdirSync(path.dirname(QQ_COOKIE_FILE), { recursive: true });
    fs.writeFileSync(QQ_COOKIE_FILE, qqCookie, { mode: 0o600 });
  } catch (e) {}
}
...
function readUpdateConfig(pkg) {
  const local = (pkg && pkg.lumaradio && pkg.lumaradio.update) || {};
  const repoHint = process.env.LUMARADIO_UPDATE_REPOSITORY
  ...
  const owner = process.env.LUMARADIO_UPDATE_OWNER || local.owner || parsed.owner || '';
  const repo = process.env.LUMARADIO_UPDATE_REPO || local.repo || parsed.repo || '';
  return {
    ...
    manifest: process.env.LUMARADIO_UPDATE_MANIFEST || '',
  };
}
...
function readUpdateMirrors(local) {
  const envMirrors = process.env.LUMARADIO_UPDATE_MIRRORS || process.env.LUMARADIO_UPDATE_MIRROR || '';
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `LumaRadio-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `LumaRadio-${latestVersion}.dmg`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('LumaRadio v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `LumaRadio/${APP_VERSION}` },
    });
...
function beatCacheRootInfo() {
  ...
  const drive = process.platform === 'win32' && root ? root.replace(/[\\\/]+$/, '').toUpperCase() : '';
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  return {
    ...
    release: {
      ...
      name: 'LumaRadio v' + latestVersion,
      version: latestVersion,
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `LumaRadio/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('LumaRadio v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `LumaRadio-${version || APP_VERSION}.dmg`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `LumaRadio-${version || APP_VERSION}.dmg`;
}
...
function writePatchFile(job, file) {
  ...
  const tmp = target + '.lumaradio-patch';
...
function normalizePatchPayload(payload) {
  ...
  if (type && type !== 'lumaradio-resource-patch') throw new Error('UNSUPPORTED_PATCH_TYPE');
...
const server = http.createServer(async (req, res) => {
  ...
  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'lumaradio',
      productName: APP_PACKAGE.productName || 'LumaRadio',
      version: APP_VERSION,
      update: {
  ...
  // ---------- 静态资源 ----------
  if (pn === '/favicon.ico') {
    serveStatic(res, path.join(STATIC_ROOT, 'icons', 'lumaradio-192.png'));
    return;
  }

  if (pn === '/NOTICE.md' || pn === '/LICENSE') {
    serveStatic(res, path.join(__dirname, pn.slice(1)));
    return;
  }

  let decodedPath = '';
  try { decodedPath = decodeURIComponent(pn === '/' ? '/index.html' : pn); }
...
server.listen(PORT, HOST, () => {
  console.log('======================================================');
  console.log(' LumaRadio Web  →  http://' + HOST + ':' + PORT);
